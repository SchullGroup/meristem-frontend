...
const SEED_AGENT_TYPES: AgentType[] = [
  ...
  { id: "AT-2", code: "STOCKBROKER",     label: "Stockbroker",      builtIn: true,  active: true },
  { id: "AT-3", code: "COLLECTING_AGENT",label: "Collecting Agent", builtIn: true,  active: true },
];
...
export interface AppState {
  ...
  updateDividendDeclaration: (id: string, updates: Partial<DividendDeclaration>) => void;
...
export const useStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      addPrincipal: (p) => set((state) => ({ principals: [...state.principals, p] })),
      updatePrincipal: (id, updates) => set((state) => ({
        principals: state.principals.map((p) => (p.id === id ? { ...p, ...updates } : p))
      })),
      addRegister: (r) => set((state) => ({ registers: [...state.registers, r] })),
      updateRegister: (id, updates) => set((state) => ({
      ...
      addAgentType: (t) => set((state) => ({ agentTypes: [...state.agentTypes, t] })),
      updateAgentType: (id, updates) => set((state) => ({
        agentTypes: state.agentTypes.map(t => t.id === id ? { ...t, ...updates } : t)
      })),
      removeAgentType: (id) => set((state) => ({
        agentTypes: state.agentTypes.filter(t => t.id !== id)
      })),
      addShareholder: (s) => set((state) => ({ shareholders: [...state.shareholders, s] })),
      updateShareholder: (id, updates) => set((state) => ({
        shareholders: state.shareholders.map((s) => (s.id === id ? { ...s, ...updates } : s))
      })),
      addCertificate: (c) => set((state) => ({ certificates: [...state.certificates, c] })),
      updateCertificate: (id, updates) => set((state) => ({
        certificates: state.certificates.map((c) => (c.id === id ? { ...c, ...updates } : c))
      })),
      addDividendDeclaration: (d) => set((state) => ({ dividendDeclarations: [...state.dividendDeclarations, d] })),
      updateDividendDeclaration: (id, updates) => set((state) => ({
        dividendDeclarations: state.dividendDeclarations.map((d) => (d.id === id ? { ...d, ...updates } : d))
      })),
      addApprovalItem: (item) => set((state) => ({ pendingApprovals: [...state.pendingApprovals, item] })),
      updateApprovalItem: (id, updates) => set((state) => ({
        pendingApprovals: state.pendingApprovals.map((a) => (a.id === id ? { ...a, ...updates } : a))
      })),
      logAudit: (entry) => set((state) => ({
        auditLog: [...state.auditLog, { ...entry, id: crypto.randomUUID(), timestamp: new Date().toISOString() }]
      })),
