...
export function ProvisionalAllotment({
  ...
  const outreachShareholders: OutreachShareholder[] = MOCK_SHAREHOLDERS.map(
    (r) => ({
      id: r.id,
  ...
  const shareholders = MOCK_SHAREHOLDERS.map((r) => ({
    ...
    if (sortKey === "holderName")
      return a.holderName.localeCompare(b.holderName) * dir;
    return (a[sortKey] - b[sortKey]) * dir;
  });
  const totalEntitlementDue = shareholders.reduce(
    (s, r) => s + r.entitlementDue,
    0,
  );
  const totalHeld = shareholders.reduce((s, r) => s + r.unitsHeld, 0);
  const totalValue =
    pricePerShare != null ? totalEntitlementDue * pricePerShare : null;
  ...
  const handleCompute = async () => {
    ...
    setComputed(true);
    toast.success(
      `Provisional allotment computed for ${MOCK_SHAREHOLDERS.length} qualifying shareholders.`,
    );
  };
  ...
  const handleValidate = () => {
    setValidated(true);
    toast.success(
      "Provisional allotment schedule validated. Pre-list generated.",
    );
  };
  ...
  const handleSendEmail = () => {
    ...
    setEmailOpen(false);
    toast.success(
      `Circular dispatched to ${MOCK_SHAREHOLDERS.length} eligible shareholders.`,
    );
  };
    ...
    return (
      <div className="space-y-4">
        <Card className="mrpsl-card p-5">
          <div
            className={`grid gap-4 text-sm mb-5 ${pricePerShare != null ? "grid-cols-3" : "grid-cols-2"}`}
          >
            <div>
          ...
          <div className="border-t border-border pt-4 flex items-center justify-between">
            <div>
              <p className="font-medium text-sm">
                Ready to compute provisional allotment
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                The system will calculate each qualifying shareholder's
                entitlement based on holdings at the qualification date (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {validated && (
          <Button
            variant="outline"
            size="sm"
            onClick={() => toast.info("Pre-list download coming soon")}
          >
      ...
      {/* Summary stats */}
      <div
        className={`grid gap-3 ${totalValue != null ? "grid-cols-4" : "grid-cols-3"}`}
      >
        {[
          {
            label: "Qualifying Shareholders",
            value: MOCK_SHAREHOLDERS.length.toLocaleString(),
          },
          {
            label: "Total Holdings (Qualification)",
            value: totalHeld.toLocaleString(),
          },
          {
            label: `Total ${entitlementLabel}`,
            value: totalEntitlementDue.toLocaleString(),
            highlight: true,
          },
          ...(totalValue != null
            ? [
          ...
          <Card key={label} className="mrpsl-card p-3">
            ...
            <p
              className={`font-mono font-semibold text-lg mt-1 ${highlight ? "text-primary" : ""}`}
            >
              {value}
            </p>
          </Card>
      ...
      <EmailPreviewModal
        ...
        contactEmail="rightssubscription@RegisPro.com"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="mrpsl-table-header">
                ...
                <th className="text-right px-4 py-2.5 font-medium">
                  <button
                    onClick={() => handleSort("unitsHeld")}
                    className="flex items-center ml-auto cursor-pointer hover:text-foreground transition-colors"
                  >
                ...
                <th className="text-right px-4 py-2.5 font-medium">
                  <button
                    onClick={() => handleSort("entitlementDue")}
                    className="flex items-center ml-auto cursor-pointer hover:text-foreground transition-colors"
                  >
            ...
            <tfoot className="border-t-2 border-border bg-muted/20">
              <tr>
                <td
                  colSpan={3}
                  className="px-4 py-2.5 text-xs font-bold text-muted-foreground text-right"
                >
