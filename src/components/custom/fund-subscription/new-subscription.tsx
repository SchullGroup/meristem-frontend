...
import { User, Users, Search, AlertCircle, CheckCircle2, Loader2 } from "lucide-react";
...
const MOCK_HOLDERS: ExistingHolder[] = [
  { id: "h1", accountNo: "FND-00123456", name: "Adebayo Oluwaseun", chn: "CHN-0012345678", email: "adebayo@email.com", currentFund: "Stanbic IBTC Dollar Fund", availableUnits: 15_000 },
  { id: "h2", accountNo: "FND-00234567", name: "Chinwe Okafor-Nwosu", chn: "CHN-0023456789", email: "chinwe@email.com", currentFund: "ARM Discovery Balanced Fund", availableUnits: 8_500 },
  { id: "h3", accountNo: "FND-00345678", name: "Emeka Nwachukwu", chn: "CHN-0034567890", email: "emeka@email.com", currentFund: "Stanbic IBTC Dollar Fund", availableUnits: 42_000 },
  { id: "h4", accountNo: "FND-00456789", name: "Fatima Garba Abubakar", chn: "CHN-0045678901", email: "fatima@email.com", currentFund: "Coronation Money Market Fund", availableUnits: 120_000 },
];

function SearchDropdown({ query, onSelect }: { query: string; onSelect: (h: ExistingHolder) => void }) {
    ...
    <div className="absolute z-10 top-full mt-1 left-0 right-0 rounded-xl border border-border bg-background shadow-md overflow-hidden">
      {matches.map((h) => (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground font-mono">{h.accountNo} · {h.chn}</p>
        </button>
      ))}
    </div>
  );
...
export function NewSubscription() {
  ...
  const canSubmitNew = !!(fundRegister && name && phone && email && bvnValid && bvn.length === 11 && units && amountPaid);
  const canSubmitExisting = !!(selectedHolder && existingUnits && existingAmount && (targetFundMode === "same" || targetFund));
  ...
  const handleReset = () => {
    setName(""); setAddress(""); setPhone(""); setEmail(""); setBvn(""); setUnits(""); setAmountPaid("");
    setSearchQuery(""); setSelectedHolder(null); setExistingUnits(""); setExistingAmount(""); setTargetFundMode("same"); setTargetFund("");
    setFundRegister(""); setSubmitted(false);
  };
  ...
  if (submitted) {
    return (
      <Card className="mrpsl-card p-8 flex flex-col items-center gap-4 text-center">
        ...
        <Button variant="outline" onClick={handleReset}>Submit Another Subscription</Button>
      </Card>
    );
  }
      ...
      <Card className="mrpsl-card p-4">
        ...
        <div className="flex gap-3">
          {(["new", "existing"] as const).map((type) => (
            ...
            >
              {type === "new" ? <User className="h-4 w-4" /> : <Users className="h-4 w-4" />}
      ...
      <div className="grid grid-cols-3 gap-5">
        <div className="col-span-2">
          <Card className="mrpsl-card p-5 space-y-4">
            ...
            {subscriberType === "new" ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input className="mrpsl-input" placeholder="e.g. John Adewale Okafor" value={name} onChange={(e) => setName(e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea className="mrpsl-input resize-none" rows={2} placeholder="Full residential address" value={address} onChange={(e) => setAddress(e.target.value)} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="mrpsl-label">Phone *</label>
                    <Input className="mrpsl-input" placeholder="+234…" value={phone} onChange={(e) => setPhone(e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="mrpsl-label">Email *</label>
                    <Input className="mrpsl-input" type="email" placeholder="subscriber@email.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="mrpsl-label">Units Subscribed *</label>
                    <Input className="mrpsl-input font-mono" type="number" min={1} placeholder="0" value={units} onChange={(e) => setUnits(e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="mrpsl-label">Amount Paid (₦) *</label>
                    <Input className="mrpsl-input font-mono" type="number" min={0} placeholder="0.00" value={amountPaid} onChange={(e) => setAmountPaid(e.target.value)} />
                  </div>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="mrpsl-label">Search Unit Holder (Account No., Name, or CHN) *</label>
                  <div className="relative">
                    ...
                    />
                    <SearchDropdown query={searchQuery} onSelect={handleSelectHolder} />
                  </div>
                </div>
                ...
                {selectedHolder && (
                  <div className="rounded-xl bg-muted/40 p-3 text-sm space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-mono text-muted-foreground">{selectedHolder.accountNo}</span>
                    </div>
                    <p className="text-xs text-muted-foreground">Current fund: {selectedHolder.currentFund}</p>
                    <p className="text-xs text-muted-foreground">CHN: {selectedHolder.chn}</p>
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-3">
                    {(["same", "different"] as const).map((mode) => (
                      ...
                      >
                        {mode === "same" ? "Same Fund (add to balance)" : "Different Fund (new entry)"}
                      </button>
                    ))}
                  </div>
                  {targetFundMode === "different" && (
                    <Select value={targetFund} onValueChange={(v) => setTargetFund(v ?? "")}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="mrpsl-label">Units Subscribed *</label>
                    <Input className="mrpsl-input font-mono" type="number" min={1} placeholder="0" value={existingUnits} onChange={(e) => setExistingUnits(e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="mrpsl-label">Amount Paid (₦) *</label>
                    <Input className="mrpsl-input font-mono" type="number" min={0} placeholder="0.00" value={existingAmount} onChange={(e) => setExistingAmount(e.target.value)} />
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="space-y-3">
          ...
          <Card className="mrpsl-card p-4 text-xs text-muted-foreground space-y-1.5">
            ...
            <p>• Existing holders subscribing to the <em>same</em> fund have their balance incremented.</p>
            <p>• Existing holders subscribing to a <em>different</em> fund get a new entry on that register.</p>
