...
import { useGetAllCertificateDemat, useSubmitForCalloverDematRequest, useRejectDematRequest } from "@/hooks/useCertDematerialisation";
import { PendingListSkeleton, DataErrorState } from "@/components/custom/ipo/loaders";
...
export default function CalloverDemat({ tab }: { tab: string }) {
    ...
    if (isError) return <DataErrorState message="Failed to load callover records." onRetry={() => refetch()} />;
    ...
    const handleBatchReject = (ids: string[], comment: string) => {
        ids.forEach(id => {
            rejectDemat({ id, data: { reason: comment } }, {
    ...
    return (
        <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
                Records submitted from Capture awaiting physical callover verification before forwarding to Authorisation.
            </p>
