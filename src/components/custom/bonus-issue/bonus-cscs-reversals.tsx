...
import { useUploadBonusReversal, useInitiateBonusReversal } from "@/hooks/useBonus";
import { DOWNLOAD_BONUS_REVERSAL_ERRORS, type BonusReversalUploadResult } from "@/actions/bonusIssuesAction";

// Bonus shares are free, so there is no FULL_REFUND path (rejected by the backend).
const RESOLUTIONS = [
  { value: "CORRECT_REBATCH", label: "Correct & Re-batch" },
  { value: "FORCE_CERTIFICATE", label: "Force Certificate" },
];

export function BonusCscsReversals({ declarationId }: { declarationId?: string }) {
  const { currentUser } = useStore();
  const [file, setFile] = useState<File | null>(null);
  const [result, setResult] = useState<BonusReversalUploadResult | null>(null);
  const [resolution, setResolution] = useState("CORRECT_REBATCH");

  const upload = useUploadBonusReversal();
  const initiate = useInitiateBonusReversal();

  if (!declarationId) {
    return (
      <Card className="mrpsl-card p-8 text-center text-sm text-muted-foreground">
        Select a bonus issue above to process CSCS responses.
      </Card>
    );
  }
  ...
  function handleUpload() {
    ...
    upload.mutate(
      { declarationId: declarationId!, file, uploadedBy: currentUser?.email ?? "" },
      {
        onSuccess: (res) => {
          const data = (res?.data ?? res) as BonusReversalUploadResult;
          setResult(data);
          toast.success(`${data.totalCredited} credited, ${data.totalErrors} error(s).`);
        },
  ...
  function handleInitiateAll() {
    ...
    initiate.mutate(
      { declarationId: declarationId!, accountNumbers: accts, resolution, initiatedBy: currentUser?.email },
      {
  ...
  async function handleDownloadErrors() {
    try {
      const blob = await DOWNLOAD_BONUS_REVERSAL_ERRORS(declarationId!);
      const url = URL.createObjectURL(blob as Blob);
      const a = document.createElement("a");
      a.href = url; a.download = `bonus_cscs_errors_${declarationId}.csv`; a.click();
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-[13px] text-muted-foreground mt-0.5">
          Upload the CSCS response file. Unsuccessful rows can be re-batched or forced (bonus shares are free — no refund path).
        </p>
      </div>
          ...
          {result.errors.length > 0 && (
            <Card className="mrpsl-card overflow-hidden">
              <div className="px-4 py-3 border-b border-border flex items-center gap-2 flex-wrap">
                ...
                <Select value={resolution} onValueChange={(v) => setResolution(v ?? "CORRECT_REBATCH")}>
                  <SelectTrigger className="mrpsl-input h-8 w-56"><SelectValue /></SelectTrigger>
                  ...
                  <tbody>
                    {result.errors.map((e, i) => (
                      <tr key={`${e.accountNumber}-${i}`} className="border-t border-border">
                        ...
                        <td className="px-4 py-2.5 text-right font-mono">{Number(e.bonusDue ?? 0).toLocaleString()}</td>
