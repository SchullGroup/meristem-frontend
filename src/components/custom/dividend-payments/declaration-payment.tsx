...
import { useInitiatePaymentRun, useApprovePaymentRun, useGetDeclarationPayment, useDownloadNibssFile } from "@/hooks/useDividendPayment";
...
export default function DeclarationPayment({ tab }: { tab: string; }) {
    ...
    const { data: declarationResponse, isLoading: fetchingPayments } = useGetDeclarationPayment({
    ...
    function initiatePaymentRun() {
        ...
        initiateMutation.mutate({
            ...
            gateway,
            initiatedBy: currentUser?.email || currentUser?.username || "ADMIN"
        }, {
            onSuccess: (res) => {
    ...
    function approvePaymentRun() {
        ...
        approveMutation.mutate({
            ...
            body: {
                ...
                authorisedBy: currentUser?.email || currentUser?.username || "ADMIN"
            }
        }, {
            onSuccess: () => {
                toast.success(
                    `Payment run submitted for ICU approval.`,
                );
    ...
    function handleDownloadNibss() {
        if (!activeRunId) {
            toast.error("Please initiate a payment run first to generate a NIBSS file.");
            return;
        }
        ...
        <>
            ...
            <Card className="p-5 border-l-4 border-primary bg-muted/10">
                <div className="flex items-center gap-8 flex-wrap">
                    ...
                    <div className="flex gap-3 items-center ml-auto flex-wrap">
                        ...
                        <Button size="lg" onClick={initiatePaymentRun} disabled={initiateMutation.isPending}>
            ...
            <div className="grid grid-cols-5 gap-3">
                <Card className="p-4">
                    ...
                    <div className="text-2xl font-mono mt-1 font-bold">{formatNumber(stats?.totalEligible || 0)}</div>
                </Card>
                <Card className="p-4">
                    <div className="mrpsl-section-title">Total Amount (₦)</div>
                    <div className="text-xl font-mono mt-1 font-bold">{formatNumber(stats?.totalPayout || 0)}</div>
                </Card>
                <Card className="p-4">
                ...
                <Card className="p-4">
                    ...
                    <div className="text-2xl font-mono mt-1 font-bold text-amber-600">
                        {formatNumber((stats?.totalEligible || 0) - (stats?.successful || 0) - (stats?.failedAttempts || 0))}
                    </div>
                </Card>
                <Card className="p-4">
            ...
            <Card className="mrpsl-card overflow-hidden">
                <div className="px-4 py-3 bg-muted/20 border-b flex items-center justify-between">
                    <span className="text-[13px] font-bold text-muted-foreground uppercase tracking-wide">
                        Payment File — {formatNumber(stats?.rows?.totalElements || 0)} records
                    </span>
                            ...
                            {fetchingPayments ? (
                                ...
                                paymentRows.map((row: any) => (
                                    <tr key={row.serial} className="hover:bg-accent/5">
                                        ...
                                        <td className="p-3">
                                            <Badge className={`border-0 text-[12px] ${row.status === "PAID" ? "bg-green-100 text-green-800" : row.status === "FAILED" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`}>
                                                {row.status}
                                            </Badge>
                                        </td>
                                    </tr>
                                ...
                                <tr>
                                    ...
                                    >
                                        {selectedRegister && selectedDiv ? "No records found matching filters." : "Select a register and dividend to load payment records."}
                                    </td>
                                </tr>
                            )}
                        </tbody>
