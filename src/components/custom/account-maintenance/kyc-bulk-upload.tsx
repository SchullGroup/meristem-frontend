...
import { useGetBulkKycChangesUploadJob, useUploadKycChanges } from "@/hooks/useAccountMaintenance";
...
export const KYCBulkUpload = ({ registerId }: KYCBulkUploadProps) => {
    ...
    const { data: jobResponse, isLoading: isJobLoading } = useGetBulkKycChangesUploadJob(
        kycUploadJobId || "",
        {
    ...
    const handleSubmit = () => {
        ...
        uploadMutation.mutate({ file: pendingFile, registerId: registerId !== "" ? registerId : undefined });
    };
                ...
                {pendingFile ? (
                    ...
                    <div className="flex items-center gap-2.5 px-3 py-2.5 border border-green-200 bg-green-50/60 rounded-xl">
                        ...
                        <div className="flex-1 min-w-0 text-left">
                            <p className="text-xs font-medium text-green-800 truncate">{pendingFile.name}</p>
                    ...
                    >
                        ...
                        <p className="text-[11px] text-muted-foreground/50 mt-1">CSV · Max 20 MB</p>
                    </div>
                )}
                    ...
                    <div className="text-left border border-border/60 rounded-xl p-4 bg-muted/20 space-y-2 relative group/job">
                        ...
                        <div className="pr-6">
                            <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
                                Job ID: <span className="font-mono text-foreground">{kycUploadJobId}</span>
                            </p>
                            ...
                            {isJobLoading && !jobData ? (
                                ...
                                <div className="mt-2 space-y-1">
                                    <div className="flex items-center gap-1.5 text-xs font-semibold">
                                        ...
                                        <span className={cn(
                                            "capitalize",
                                            jobData?.status?.toUpperCase() === "COMPLETED" && "text-green-600",
                                            jobData?.status?.toUpperCase() === "FAILED" && "text-red-600",
                                            !["COMPLETED", "FAILED"].includes(jobData?.status?.toUpperCase() || "") && "text-amber-500 animate-pulse"
                                        )}>
                                    ...
                                    {jobData && (
                                        <p className="text-[11px] text-muted-foreground">
                                            Processed: <strong>{jobData.processed}</strong> / {jobData.totalRows} rows
                                            {jobData.succeeded > 0 && <span className="text-green-600 ml-1.5">({jobData.succeeded} s)</span>}
                                            {jobData.failed > 0 && <span className="text-red-600 ml-1.5">({jobData.failed} f)</span>}
                                        </p>
                                    )}
                                </div>
                            )}
