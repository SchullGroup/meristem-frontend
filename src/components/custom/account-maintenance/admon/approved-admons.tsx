...
export default function ApprovedAdmons({ tab }: { tab: string }) {
  ...
  function handleReverseAdmon() {
    if (!selected) {
      toast.error("Please select an ADMOR to reverse");
      return;
    }
    ...
    initiateReversalMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("ADMOR reversal initiated");
          closeModal();
          refetch();
        },
        onError: (err) => {
    ...
    <>
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              Initiate Reversal for {selected?.estateNamePreview}
            </DialogTitle>
          </DialogHeader>
          {selected && (
            <div className="space-y-4 p-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  variant="secondary"
                  onClick={handleReverseAdmon}
                  disabled={initiateReversalMutation.isPending}
                >
                  {initiateReversalMutation.isPending
                    ? "Processing..."
                    : "Reverse Selected"}
                </Button>
              </div>
