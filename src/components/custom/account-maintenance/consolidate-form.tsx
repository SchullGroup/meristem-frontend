...
export default function Consolidate({ tab }: { tab: string }) {
  ...
  function handleSubmitSingle() {
    ...
    if (selectedSourceIds.size === 0) {
      toast.error("Add at least one source account.");
      return;
    }
    if (!destinationId) {
    ...
    const selectedRegisterObj = activeRegisters?.content.find(
      (r) => r.registerId === register,
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 flex-wrap">
        <Select
          value={register}
          onValueChange={(value) => setRegister(value || "")}
        >
                    ...
                    <tr>
                      <th className="p-2">
                        <Checkbox
                          checked={
                            sourceAccounts.length > 0 &&
                            sourceAccounts.every((a) =>
                              selectedSourceIds.has(a.holderId),
                            )
                          }
                          onCheckedChange={(checked) => {
                            if (checked) {
                              setSelectedSourceIds(
                                new Set(sourceAccounts.map((a) => a.holderId)),
                              );
                            } else {
                              setSelectedSourceIds(new Set());
                            }
                      ...
                      <tr key={acc.holderId} className="mrpsl-table-row">
                        <td className="p-2">
                          <Checkbox
                            ...
                            onCheckedChange={() =>
                              toggleSourceSelect(acc.holderId)
                            }
                          />
                        </td>
              ...
              {sourceAccounts.length > 0 && (
                <div className="bg-muted/20 p-4 rounded-md space-y-3">
                  <div className="text-[13px] text-muted-foreground">
                    Merging {selectedSourceIds.size} account
                    {selectedSourceIds.size !== 1 ? "s" : ""} into destination.
                  </div>
                  <div className="text-[13px] text-muted-foreground">
