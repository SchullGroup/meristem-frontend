...
import { RejectConsolidation, ReviewConsolidation } from "./review-consolidation";
...
export default function PendingConsolidationApprovals() {
    ...
    const { data, isLoading, isError, error, refetch } = useGetAllCertConsolidations({
        page: page,
        pageSize: pageSize,
        status: "PENDING"
    });
    ...
    const visibleConsolIds = pendingConsolidations.map((r: CertificateConsolidation) => r.id);
                            ...
                            <tbody className="divide-y text-[13px]">
                                {pendingConsolidations?.map((row) => (
                                    <tr key={row.id} className="mrpsl-table-row">
                                        ...
                                        <td className="p-3 text-muted-foreground">{formatDate(row.submittedAt)}</td>
