...
import { useSubmitConsolidationRequest } from "@/hooks/useCertConsolidation";
import { formatDate } from "@/lib/utils/format";



export default function NewConsolidation({ setTab }: { setTab: (tab: string) => void }) {
        ...
        <div className="mt-6">
            ...
            <Card className="mrpsl-card p-4">
                <div className="flex items-end gap-3">
                    ...
                    <Button size="xl" onClick={handleLoadCertificates} disabled={isLoading}>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="h-10 w-10 rounded-full bg-green-600 flex items-center justify-center shrink-0">
                                <span className="text-white font-bold text-sm">
                                    {firstCert?.shareholderName ? firstCert.shareholderName.substring(0, 2).toUpperCase() : "SH"}
                                </span>
                            </div>
                            ...
                            <div className="text-right shrink-0">
                                ...
                                <div className="font-mono text-sm font-bold text-green-900">
                                    {certList.reduce((acc, cert) => acc + (cert.units || 0), 0).toLocaleString()} total units
                                </div>
                            </div>
                        </div>
                    </Card>
                            ...
                            <thead className="mrpsl-table-header">
                                <tr>
                                    <th className="p-3 w-10">
                                        <Checkbox
                                            checked={certList.length > 0 && selectedIds.size === certList.length}
                                            onCheckedChange={() => toggleSelectAll(certList.map(c => c.id))}
                                        />
                                    </th>
                            ...
                            <tbody className="divide-y">
                                {certList.map((c) => (
                                    <tr key={c.id} className="hover:bg-accent/5">
                                        ...
                                        <td className="p-3 font-mono text-[13px]">{c.certNumber}</td>
                                        <td className="p-3 font-mono text-right">{c.units?.toLocaleString()}</td>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-[13px] text-muted-foreground tabular-nums mt-0.5">
                                            {certList.filter(c => selectedIds.has(c.id)).reduce((acc, cert) => acc + (cert.units || 0), 0).toLocaleString()} total units
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="space-y-4 p-4">
                        <div className="space-y-2">
                            <label className="mrpsl-label">
                                New Certificate Number <span className="text-destructive">*</span>
                            </label>
                            <Input
                                value={newCertNumber}
                        ...
                        <div className="space-y-2">
                            <label className="mrpsl-label">
                                Reason for Consolidation <span className="text-destructive">*</span>
                            </label>
                            <Textarea
                                value={reason}
