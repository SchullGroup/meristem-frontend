...
export default function UpdateReconciliation({ tab }: { tab: string }) {
    ...
    const { data, isLoading, isError, error, refetch } = useReconciliationFlaggedTransactions({
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex gap-2 items-center flex-wrap">
                ...
                <Select
                    value={register}
                    onValueChange={(v) => setRegister(v || "All")}
                >
                    ...
                    <SelectContent>
                        ...
                        {activeRegisters?.content?.map((r) => (
                            <SelectItem key={r.registerName} value={r.registerId}>
                                        ...
                                        Array.isArray(data?.content) && data?.content?.length > 0 ?
                                            data?.content?.map((row) => (
                                                <tr key={row.id} className="mrpsl-table-row">
                                                    ...
                                                    <td className="px-4 py-3 font-semibold">{row.chn}</td>
                                                    <td className="px-4 py-3 tabular text-[13px]">{row.holderName}</td>
                                                    ...
                                                    <td className="px-4 py-3 text-right tabular">{formatNumber(row.holdings)}</td>
                                                    <td className="px-4 py-3 text-right tabular text-amber-600 font-semibold">
                                                        {formatNumber(row.shortfall)}
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        <Badge className={`border-0 text-[13px] ${row.status === "PENDING" ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}`}>
                                                            {row.status}
                                                        </Badge>
                                                    </td>
                        ...
                        <Card className="mrpsl-card p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <DateInput label="Transaction Date" date={txDate} setDate={setTxDate} />
                                </div>
                                <div className="space-y-2">
                                    <label className="mrpsl-label">Transfer No</label>
                                    <Input name="transferNo" value={transferNo} onChange={e => setTransferNo(e.target.value)} placeholder="TRN-0099123" className="mrpsl-input" />
                                </div>
                                <div className="space-y-2">
                                    <label className="mrpsl-label">Units</label>
                                    <Input name="units" type="number" value={txUnits} onChange={e => setTxUnits(Number(e.target.value))} className="mrpsl-input tabular" />
                                </div>
                                <div className="space-y-2">
