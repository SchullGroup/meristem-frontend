...
import { CheckCircle, Loader2, Check, Search, MapPin } from "lucide-react";
...
import { useGetHolders, useUpdateHolderStates } from "@/hooks/useCscs";
...
export function PreviewHolders({
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      toast.success(
        `Successfully committed updates for Page ${currentPage + 1}.`,
      );
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Filters */}
      <div className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-2 items-center w-3/4">
        ...
        <div className="ml-auto grid grid-cols-[1fr_2fr] items-center gap-5">
          <div className="text-[13px] text-center text-muted-foreground">
            <span className="text-primary font-semibold">{totalConfirmed}</span>{" "}
            / {holdersData?.content?.length || 0} confirmed
          </div>
          {filteredHolders.some((h) => confirmedStates[h.id] === undefined) && (
            <Button
              size="sm"
              variant="outline"
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
        ...
        <p className="text-[13px] text-emerald-800">
          ...
          approach significantly reduces manual review time and allows hundreds
          of records to be processed at once.
        </p>
      </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border/60">
              {isLoading ? (
                ...
                <DataErrorState
                  message={error?.message || "Failed to fetch holders"}
                  onRetry={refetch}
                />
              ) : filteredHolders?.length > 0 ? (
                filteredHolders.map((h) => {
                  const confirmedState = confirmedStates[h.id] !== undefined;
                  return (
                            ...
                            {!confirmedState ? (
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  toast.success(`${h.name} state confirmed`);
                                }}
                              >
