...
const SEED_BANK_CHANGES: BankChange[] = [
  {
    id: "1",
    chn: "C0098765AK",
    holderName: "MICHAEL OBINNA UKPONG",
    register: "MTNN",
    field: "BANK_NAME",
    oldValue: "First Bank of Nigeria",
    newValue: "Access Bank PLC",
  },
  {
    id: "2",
    chn: "C0098765AK",
  ...
  },
  {
    ...
    register: "UBA",
    field: "BANK_NAME",
    oldValue: "Guaranty Trust Bank",
    newValue: "Zenith Bank PLC",
  },
  {
    id: "4",
    chn: "C0087654BK",
  ...
  },
  {
    ...
    newValue: "22345678901",
  },
  {
    id: "6",
    chn: "C0065432DK",
    holderName: "SEUN ADESANYA FATOUMBI",
    register: "DANGCEM",
    field: "BANK_NAME",
    oldValue: "Stanbic IBTC Bank",
    newValue: "United Bank for Africa",
  },
  {
    id: "7",
    chn: "C0054321EK",
    holderName: "CHIDINMA GRACE OBIORA",
    register: "SEPLAT",
    field: "ACCOUNT_NUMBER",
    oldValue: "0456789012",
    newValue: "0654321098",
...
const grouped = SEED_BANK_CHANGES.reduce<Record<string, BankChange[]>>(
  (acc, c) => {
...
export function StepReviewBankChanges({
  batchRef: _batchRef,
  onProceed,
}: StepReviewBankChangesProps) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          The following records have updates to their bank details (BVN, Bank
          Name, Account Number). To view more details, go to the KYC mandate
          page.
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
        ...
        <p className="text-sm text-blue-800">
          <strong>This screen is read-only.</strong> Bank mandate changes (BVN,
          Bank Name, Account Number) are <em>never</em> auto-applied from a CSCS
          batch — they have been raised as KYC update requests for the normal
          maker-checker approval workflow.{" "}
          <strong>
            The only field this pipeline writes directly is the STATE column
            (Step 2).
          </strong>
        </p>
      </div>
      ...
      <p className="text-sm text-muted-foreground">
        <span className="font-semibold text-foreground">
          {SEED_BANK_CHANGES.length}
        </span>{" "}
        bank mandate change{SEED_BANK_CHANGES.length !== 1 ? "s" : ""} detected
        across{" "}
        <span className="font-semibold text-foreground">
              ...
              {GROUPED_ENTRIES.map(([chn, changes]) =>
                changes.map((c, idx) => (
                  <tr key={c.id} className="mrpsl-table-row">
                    ...
                    {idx === 0 && (
                      <td
                        className="px-4 py-3 font-mono text-[13px] text-muted-foreground"
                        rowSpan={changes.length}
                      >
                        {chn}
                      </td>
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        className={`border-0 text-[12px] ${FIELD_COLOR[c.field]}`}
                      >
                        {FIELD_LABEL[c.field]}
                      </Badge>
                    </td>
      ...
      <div className="flex items-center justify-between pt-2">
        <Button
          variant="outline"
          onClick={() => router.push("/certificates/kyc")}
        >
