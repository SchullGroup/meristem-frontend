...
const SEED_HOLDERS: HolderRecord[] = [
  { id: "1",  register: "DANGCEM", chn: "C0012345AK", name: "JOHN ADEYEMI BABATUNDE",   address: "14 Allen Avenue, Ikeja, Lagos Island",     actualState: "Lagos",   gisState: "Lagos"  },
  { id: "2",  register: "DANGCEM", chn: "C0023456BK", name: "NGOZI CHIDINMA OKAFOR",    address: "45 Trans-Ekulu Road, Enugu North",          actualState: "UNKNOWN", gisState: "Enugu"  },
  { id: "3",  register: "DANGCEM", chn: "C0033211DK", name: "TUNDE ABIODUN SALAMI",     address: "9 Taiwo Street, Ilorin",                   actualState: "UNKNOWN", gisState: "Kwara"  },
  { id: "4",  register: "MTNN",    chn: "C0034567CK", name: "SAMUEL OLUWASEUN ADELEKE", address: "23 Old Bodija Estate, Ibadan",              actualState: "UNKNOWN", gisState: "Oyo"    },
  { id: "5",  register: "MTNN",    chn: "C0045678DK", name: "FATIMA ABUBAKAR MUSA",     address: "12 Ahmadu Bello Way, Kaduna South",        actualState: "Kaduna",  gisState: "Kaduna" },
  { id: "6",  register: "MTNN",    chn: "C0046789EK", name: "ADAEZE CHIBUIKE IROEGBU",  address: "3 Aba Road, Port Harcourt",                actualState: "UNKNOWN", gisState: "Rivers" },
  { id: "7",  register: "SEPLAT",  chn: "C0056789EK", name: "EMEKA CHUKWUEMEKA EZE",    address: "8 Owerri Road, Owerri North",               actualState: "UNKNOWN", gisState: "Imo"    },
  { id: "8",  register: "SEPLAT",  chn: "C0067890FK", name: "AMAKA NGOZI OKONKWO",      address: "5 Onitsha Road, Onitsha North",             actualState: "Anambra", gisState: "Anambra"},
  { id: "9",  register: "UBA",     chn: "C0078901GK", name: "IBRAHIM USMAN HASSAN",     address: "7 Zaria Road, Kano Municipal",              actualState: "UNKNOWN", gisState: "Kano"   },
  { id: "10", register: "UBA",     chn: "C0089012HK", name: "BLESSING CHISOM NWOSU",    address: "22 Airport Road, Port Harcourt",            actualState: "Rivers",  gisState: "Rivers" },
  { id: "11", register: "UBA",     chn: "C0099123IK", name: "CHUKWUEMEKA OKAFOR",       address: "14 Agbani Road, Enugu South",               actualState: "UNKNOWN", gisState: "Enugu"  },
  { id: "12", register: "UBA",     chn: "C0109234JK", name: "YETUNDE ADEFOPE ADEYEMI",  address: "6 Ring Road, Ibadan North",                 actualState: "UNKNOWN", gisState: "Oyo"    },
];
...
export function StepResolveStates({ batchRef: _batchRef, onComplete }: StepResolveStatesProps) {
  ...
  const missingCount = SEED_HOLDERS.filter((h) => resolvedForRow(h) === null).length;

  const confirmedCount = SEED_HOLDERS.filter((h) => resolvedForRow(h) !== null).length;

  const filtered = useMemo(() => {
    return SEED_HOLDERS.filter((h) => {
      if (registerFilter !== "All" && h.register !== registerFilter) return false;
  ...
  const acceptAllGis = () => {
    ...
    SEED_HOLDERS.forEach((h) => { if (h.actualState === "UNKNOWN") next[h.id] = h.gisState; });
  ...
  const revertGis = () => {
    ...
    SEED_HOLDERS.forEach((h) => {
      if (next[h.id] === h.gisState && h.actualState === "UNKNOWN") delete next[h.id];
    });
  ...
  const handleCommit = () => {
    if (missingCount > 0) {
      toast.error(`YOU STILL HAVE ${missingCount} UNKNOWN STATE${missingCount !== 1 ? "S" : ""}. Resolve all before saving.`);
      return;
    }
    toast.success(`Shareholder records updated — ${SEED_HOLDERS.length} rows committed.`);
    onComplete();
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex-wrap gap-3">
        <div className="flex items-center gap-2 text-sm text-blue-900">
          ...
          <span>
            ...
            &nbsp;·&nbsp;
            <strong>{SEED_HOLDERS.filter((h) => h.actualState === "UNKNOWN").length} GIS SUGGESTED STATES</strong>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {missingCount > 0 && (
            <>
              <Button size="sm" variant="outline" onClick={acceptAllGis}>
                Accept All GIS Suggestions
              </Button>
              <Button size="sm" variant="outline" onClick={() => {
                // Accept GIS across all registers
                acceptAllGis();
              }}>
                Accept GIS Across Registers
              </Button>
            </>
          )}
          {gisAccepted && (
            <Button size="sm" variant="ghost" className="text-muted-foreground" onClick={revertGis}>
      ...
      <div className="flex gap-2 items-center flex-wrap">
        ...
        <span className="ml-auto text-[13px] text-muted-foreground">
          <span className="text-primary font-semibold">{confirmedCount}</span> / {SEED_HOLDERS.length} confirmed
        </span>
      </div>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
        ...
        <p className="text-[13px] text-emerald-800">
          ...
          <strong>Accept All GIS Suggestions</strong> before saving. State is mandatory for
          legal compliance — you cannot save while any UNKNOWN states remain.
        </p>
      </div>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1.5">
                        <Select
                          ...
                          onValueChange={(v) => {
                            if (!v) return;
                            setConfirmedStates((prev) => ({ ...prev, [h.id]: v }));
                        ...
                        >
                          ...
                          >
                            <SelectValue placeholder={isUnknown ? h.gisState + " (GIS)" : h.actualState} />
                          </SelectTrigger>
                        ...
                        {isUnknown && !isConfirmed && (
                          <Button
                            ...
                            onClick={() => { acceptGis(h); toast.success(`${h.name} → ${h.gisState} (GIS)`); }}
                          >
                      ...
                      {isConfirmed && isUnknown && confirmedStates[h.id] !== h.gisState && (
