...
const getXUserHeader = () => {
    ...
    return { 'X-User': currentUser?.email || `${currentUser?.firstName} ${currentUser?.lastName}` || 'Unknown' };
};
...
export interface DematParams {
    ...
    chn?: string
    holderName?: string
    status?: 'DRAFT' | 'CALLOVER' | 'AUTHORISED' | 'ICU_APPROVED' | 'LODGED' | 'REJECTED'
    dateFrom?: string
    dateTo?: string
    page?: number
    size?: number
...
export const getAllCertificateDemat = async (params?: DematParams) => {
    try {
        const response = await api.get<ContentPaginatedResponse<Demat>>(
            `/demat`,
            { params },
        );
...
export const captureDematRequest = async (data: CaptureDematRequest) => {
    try {
        const res = await api.post<Demat>(`/demat`, data, { headers: getXUserHeader() });
        return res.data;
    } catch (error) {
        const err = error as ErrorLike;
        throw new Error(returnErrorMessage(err));
...
export const rejectDematRequest = async (id: string, data: { reason: string }) => {
    try {
        const res = await api.patch<Demat>(`/demat/${id}/reject`, data, { headers: getXUserHeader() });
        return res.data;
    } catch (error) {
        const err = error as ErrorLike;
        throw new Error(returnErrorMessage(err));
...
export const lodgetDematRequest = async (id: string, data: {
    reason: {
    ...
    try {
        const res = await api.patch<Demat>(`/demat/${id}/lodge`, data, { headers: getXUserHeader() });
        return res.data;
    } catch (error) {
        const err = error as ErrorLike;
        throw new Error(returnErrorMessage(err));
...
export const icuApproveDematRequest = async (id: string) => {
    try {
        const res = await api.patch<Demat>(`/demat/${id}/icu-approve`, {}, { headers: getXUserHeader() });
        return res.data;
    } catch (error) {
        const err = error as ErrorLike;
        throw new Error(returnErrorMessage(err));
...
export const submitForCalloverDematRequest = async (id: string) => {
    try {
        const res = await api.patch<Demat>(`/demat/${id}/callover`, {}, { headers: getXUserHeader() });
        return res.data;
    } catch (error) {
        const err = error as ErrorLike;
        throw new Error(returnErrorMessage(err));
...
export const authorizeDematRequest = async (id: string) => {
    try {
        const res = await api.patch<Demat>(`/demat/${id}/authorise`, {}, { headers: getXUserHeader() });
        return res.data;
    } catch (error) {
        const err = error as ErrorLike;
        throw new Error(returnErrorMessage(err));
